                    ...
                    <div class="table-responsive theme-scrollbar">
                        <div id="basic-9_wrapper" class="dataTables_wrapper">
                            <div id="basic-9_filter"
                                ...
                                <div>
                                    <a href="{{route('admin.jubilees.create')}}" class="btn btn-outline-success" type="button" data-bs-original-title="" title="">Добавить</a>
                                </div>
                                ...
                                @if(isset($jubilees))
                                    @foreach($jubilees as $jubilee)
                                        <tr role="row" >
                                            <td>{{$jubilee->id}}</td>
                                            <td>{{$jubilee->name}}</td>
                                            <td>{{$jubilee->created_at}}</td>
                                            <td>
                                                <ul class="action">
                                                    <li class="edit"><a href="{{route('admin.jubilees.edit', $jubilee->id)}}" data-bs-original-title="" title=""><i
                                                                class="icon-pencil-alt"></i></a></li>
                                                    <li class="delete">
                                                        <form action="{{route('admin.jubilees.delete', $jubilee->id)}}" method="post">
                                                            @csrf
                                                            @method('delete')
                                                            <button type="submit"
